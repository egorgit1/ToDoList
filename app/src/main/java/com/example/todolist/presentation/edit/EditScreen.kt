...
fun EditScreenContent(
    ...
    var selectedText by remember { mutableStateOf("Изменить цвет") }
    Column(
        modifier = Modifier
            .fillMaxSize(),
        ...
        Box(
            ...
            IconButton(
                onClick = { onEvent(EditScreenEvent.ExpandChanged(true)) },
                modifier = Modifier.align(Alignment.CenterEnd)
            ) {
                Icon(Icons.Default.ArrowDropDown, contentDescription = stringResource(R.string.change_color))
            }
            DropdownMenu(
                expanded = state.isExpanded,
                onDismissRequest = { onEvent(EditScreenEvent.ExpandChanged(false)) }
            ) {
                mapOfColors.forEach { item ->
                    DropdownMenuItem(
                        onClick = {
                            selectedText = item.key
                            onEvent(EditScreenEvent.ExpandChanged(false))
                            onEvent(EditScreenEvent.ColorChanged(item.value))
                        },
                        text = {
                            Text(
                                text = item.key
        ...
        OutlinedTextField(
            ...
            onValueChange = { onEvent(EditScreenEvent.DescriptionChanged(it)) },
            placeholder = {
                Text(
        ...
        BackHandler {
            onEvent(EditScreenEvent.BackToFeedBtnClicked(id))
